...
/*What was the maximum commission paid in the customer table? The average?*/
...
/*Write a query to that returns the customer_id, business_type and Country from the customer table.*/
 ...
 from (select customer_id,count(service_completed) as serviceshired
 	   from billedservices
 	   group by customer_id
       having serviceshired >1)
 
 /*How many customers from OUTSIDE the United States have an entry in the billed services table?*/
 
SELECT COUNT(billedservices.customer_id) AS cuenta          
FROM billedservices
INNER JOIN customer        
ON billedservices.customer_id=customer.Customer_id AND
